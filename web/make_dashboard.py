...
def loss_chart(run: Run) -> str:
    title = "Training loss"
    subtitle = "Total, policy, value (and margin, final-wall) loss over self-play games."
    ...
    # "margin" only exists from run4 on and "final walls" from run6 on; a run
    # without one simply has no such points and the series drops out below.
    defs = [
        ("total", "loss", SERIES[2]),
        ("policy", "loss_p", SERIES[0]),
        ("value", "loss_v", SERIES[1]),
        ("margin", "loss_m", SERIES[3]),
        ("final walls", "loss_a", SERIES[4]),
    ]
    present = []
