...
| **Porcelain** | **2564** | W160 body (3.9M) pretrained on the 7M teacher's 1024-sim searched self-play + every position of the expert games searched by the same teacher (`docs/PORCELAIN.md`, `docs/superpowers/specs/2026-09-05-cloud-selfplay-design.md`) | **default opponent** |
...
strongest, over ≥300 games at matched think time** (`docs/BOT_DEPLOYMENT.md`).
...
- **Compute is now cheap.** The Rust engine (`docs/RUST_ENGINE.md`) makes an
...
1. **A genuinely stronger teacher** (the Lapis Lazuli road, ~$20-30, two
   days). Polish the 19.5M seed (`runs/big_t/checkpoints/ckpt-000000.pt`,
   ...
   W160 body with `scripts/porcelain_pretrain.sh`, then gate. Expected: the
   student gains what the teacher gained beyond Porcelain's search.
2. **Strategy from the expert games** (the task force,
   `docs/STRATEGY_TASKFORCE.md`, $50). The hypothesis is Rémi's: elite
...
3. **A faster student, not a bigger one.** The browser fixes the student's
   ...
   latency, or tree reuse between moves in the browser (`web/player/js/mcts.js`
   ...
   (`web/player/test/selfplay.test.mjs`).
...
- `NOTES_FOR_REMI.md`, newest on top, is the ledger of results; negative
...
- Other agents share this repo; commit path-scoped; never touch a running
  run's directory or `data/human/` state.
